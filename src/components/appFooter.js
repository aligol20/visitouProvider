...
export default class AppFooter extends Component {
    ...
    componentWillMount(){
        AsyncStorage.getItem('AmIlogged', (err, store) => {
            ...
            let aan = {provider: JSON.parse(store).user};
            ...
            fetch('http://visitou.ir/api/readRequestMount.php', {
                ...
                response.json()
                    .then((responseJsonMM) => {
                        console.log(responseJsonMM.length, 'fff000zaqwer');
                        if (responseJsonMM.length) {
                            badgNumber = responseJsonMM.length;
                            this.setState({badgebadge: badgNumber});
                            console.log('slkvkljd', badgNumber);
                            if (badgNumber === '0') {
                                this.setState({koooni: false})
                            } else {
                                this.setState({koooni: true})
    ...
    render() {
        //console.log('dcmkdfmf',Actions.currentScene);
        function tabAction(tab) {
            let pp=Actions.currentScene;
            //console.log('skljdf',tab);
            // this.setState({activeTabName:tab});
            //console.log('kkkk','kdkdkdkdk');
                switch (tab) {
                    case 'feed':
        ...
        let pp=Actions.currentScene;
        //console.log(Actions.currentScene,'kjfkdkfk');
        switch (pp){
            case 'feed':
                return (<Footer >
                    <FooterTab style={{backgroundColor: '#1c375c'}}>
                        ...
                        <IconBadge
                            ...
                            BadgeElement={
                                <Text style={{color: '#FFFFFF', fontSize: 11,fontFamily:'B Koodak'}}>{this.state.badgebadge}</Text>
                            }
                            IconBadgeStyle={
                                {
                            ...
                            Hidden={this.state.koooni === false}

                        />
                        <Button

                            onPress={() => tabAction('feed')}>
                            <Icon5 color={'#fdb913'}
                                   name="home" size={27}/>
